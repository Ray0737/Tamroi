...
- `assets/street-quest/`: generated, web-optimized Bangkok explorer collage, route paper, cream/ink/map/kraft UI materials, class badge stamps/paper label, generated Mission/Watchtower ink stamps, generated guild seal/journal/control tickets, field-kit slips, image-backed numbered mission passes, join-request action tickets, figure capture/dossier/field-sheet/settings/notification templates, and tab icons, Collection archive collage, and Community podium collage used by the runtime theme and transient field UI.
...
- `js/coop.js`: collaborative image-backed numbered mission passes and real-time check-in progress; Jigsaw cards reuse the same readable pass shell.
...
- `restyling/` remains the concept reference; the full runtime now loads `css/street-quest.css`, generated Bangkok collage/route assets, four generated UI materials, generated blue/red ink stamp elements, generated guild compass seal, torn journal label, action tickets, danger ticket, roster/note slips, image-backed numbered co-op pass variants, readable join-request action tickets, figure capture/reveal and dossier templates, compact Support Node/Watchtower paper tickets, field-kit/settings/notification templates, and custom tab icons, a Collection archive collage, and a Community three-poster podium collage while preserving existing behavior and DOM hooks. Collection uses a dark archive page with a torn toolbox and poster cards; Community separates the current-player rank ticket from a compact dark leaderboard, uses an ink group ledger and parchment forum, gives My Group an ink field-crew dossier, kraft-paper invite/member ticket, image-generated paper controls, note cards, mission passes, and field-journal icons, and repeats the current player in the ranked list at their actual position with a highlighted row. Capture reveals, figure detail, C-class encounter, Lore, settings, notifications, and the relationship graph use generated raster surfaces with live HTML text layered in readable zones. Eleven browser-verified mobile captures, including populated My Group, Forum, and Watchtower Check-In states, are stored in `restyling/05_bangkok_street_quest/screenshots/`; the complete set was regenerated on 2026-07-13 with the Playwright visual suite.
...
- Watchtower Check-In now renders as the compact `class-label-paper.png` ticket with a dark district pass, stamped Support requirements, ready Encounter card, and paper Cancel control; Playwright verifies the 411×800 composition; the check-in state flow is unchanged.
- Support Node sheets now use the compact `class-label-paper.png` ticket with a bordered location block, visited stamp, route metadata, and paper Close control; Playwright verifies the 412×700 composition; `MapModule.visitSupportNode()` behavior is unchanged.
...
- Settings now uses an identity-page passport layout with passport metadata, an issued stamp, a field-permit strip, and a departure action; the portrait, identity lines, permit rows, version strip, and sign-out control are aligned to the generated field-kit artwork at mobile widths.
...
- Figure capture/reveal and C-class encounter states now use generated poster templates for every figure class; live names, portraits, points, and actions remain DOM content, and the figure detail modal/relationship graph use matching dossier and field-map surfaces.
- Settings GPS permission now renders granted state as a generated blue ink stamp instead of a plain `อนุญาต ✓` label; settings, notifications, Lore, and field sheets keep their existing controls and APIs.
...
- Guild co-op mission rendering now uses numbered blue/pink/lime mission rails, high-contrast Thai copy, explicit progress percentages, and full-width orange check-in actions in both the Mission tab and My Group panel; Jigsaw cards keep their existing interaction flow inside the shared pass shell.
