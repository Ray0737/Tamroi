...
> Last accurate as of: 2026-07-04 — re-verified against live code (`js/`) and the live Supabase DB (project `NSC`), not just prior notes.
...
- Most figures (48 of 72 live figures — two-thirds) have no saved map coordinates yet. They still show a dashed "?" fallback marker at their district's watchtower spot (see Bugs & Fixes Needed) rather than disappearing, but a real pinned location is still missing for most of the roster.
...
## Bugs & Fixes Needed (cross-checked against the live game code and the live database, 2026-07-04)

- ✅ **Fixed — Figures without a map location now show a fallback marker.** A figure missing lat/lng no longer silently disappears — it now renders a dashed "?" pin at its district's watchtower spot with a "ตำแหน่งยังไม่ระบุ" tooltip, and the missing IDs are logged to the console. The underlying content gap is still large though — 48 of 72 live figures (two-thirds) still have no real coordinates, so most figures a player finds are on this fallback pin, not their actual location.
...
- **Rebalance common vs. gated captures (target 80% ungated / 20% gated)** — ⚠️ **Still far from the target, unchanged since last check.** The gating mechanism itself works correctly. Live-verified split: 11 S-tier + 20 A-tier = 31 gated figures against 21 B-tier + 20 C-tier = 41 ungated, out of 72 total — roughly 43% gated / 57% ungated. Better than 50/50, but still far short of 80/20. More B/C-tier figures (or reclassifying some A-tier ones) are still needed to hit the target.
