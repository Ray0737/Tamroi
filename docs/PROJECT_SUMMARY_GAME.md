...
- **Onboarding** — pick a home district → `confirmHomeAndStart()` awards a one-time +50 point welcome bonus, and the very next app.html load shows a celebratory reveal card for it (gated by a one-shot `sessionStorage` flag, so it never repeats on later logins).
...
- **Jigsaw missions** — a group-only mission type where each member is assigned one chapter of a 3-part historical timeline. To contribute, a member must physically walk to their chapter's GPS checkpoint, pass a short recall quiz, then submit a structured summary (time period, key figure, event). Once everyone's submitted, the group drag-reorders all chapters into the correct chronological order and votes; a unanimous correct order wins the mission for everyone. Every Jigsaw card shows a live **20-minute countdown** (starts from the first member's assignment) and an always-visible **team overview** listing each member's current status (locked / writing summary / submitted / voted), so nobody has to guess how the rest of the group is doing.
