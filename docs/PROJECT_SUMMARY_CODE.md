...
| Fog of War — walk cells (`_revealWalkCell`) | ✅ Complete (per-account since 2026-07-13) | Ignores fixes with accuracy >100m (fixed); storage key bump wiped old trails once | Synced to `user_walk_points` (own-row RLS); localStorage kept as a per-user-keyed offline cache (legacy shared key migrated once, claimed by the first account to boot) |
