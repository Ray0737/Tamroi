...
> **Latest:** map engine replatformed from Leaflet to MapLibre GL for a 45° tilted camera view, locate-me FAB (flies to live GPS), map zoom lock, real-time fog sync across devices with Map Discovery % written back to the profile, Collection class filter as a dropdown with a combinable "Owned" toggle, C-Class proximity capture (80m radius circle, always-visible markers, tap-to-capture sheet) with new C-Class figures, captured figures hidden from the map at init, walk-cell fog gated on GPS accuracy ≤100m, lore unlocks persisting to `user_lore` correctly, light-theme map (white CartoDB tiles, darker Fog of War), figure cameo on lore chain completion, watchtower check-in Encounter Key gating A-tier encounters, forum moderation with probation, and a PDPA age-consent step at onboarding.
